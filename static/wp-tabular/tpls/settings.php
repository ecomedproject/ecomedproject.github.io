<?php
    $shortcode = array();
    $code = array();
    $shortcode=get_option("_wptb_shortcode");
...
<style type="text/css">
   ...
   <h2>Wp Table Settings</h2> <br>
   <b>Short-codes will be replaced by short-code value while rendering table at frontend. Use image url or any text for shortcode value.</b>
   <div style="clear: both;"></div>
   <form action="" method="post" id="wptb" style="float: left;width:500px;">
    <input type="hidden" name="action" value="wptb_save_shortcode">
...
<div title="Click to toggle" class="handlediv"><br></div><h3 class="hndle"><span>Custom Icon Short Codes</span></h3>
<div class="inside">
...
<?php
    if(is_array($shortcode)){
        ...
        foreach($shortcode as $key => $value){
            $i++;
         ?>
         <tr id="<?php echo "row_".$i;?>"><td valign="top"><input size="15" type="text" name="code[<?php echo $key;?>]" value="<?php echo $code[$key];?>" /></td><td align="left"><input size="80" type="text" name="shortcode[<?php echo $key;?>]" value="<?php echo $value;?>" /></td><td><img class='deleterow' rel='<?php echo "row_".$i;?>' title='Delete this row' src='<?php echo plugins_url();?>/wp-tabular/images/delete.png' /></td></tr>
         <?php   
        }
    }
?>
...
<input class="button" type="button" name="addnew" id="addnew" value="ADD NEW">
    <br clear="all"></div>
</div>  </div>
